...
def train(model, args, epochs=10, experiment_name="DeepLab", lr=0.0001, root=".", device=None,
    ...
    #print(image_size)
    adaptive_image_loss_func = AdaptiveImageLossFunctionSkewedNew(image_size, np.float32, 0, beta_lo=0.001, beta_hi=1.999, beta_init=0.1, scale_lo=1.0, scale_init=1.0)
    ################################################################################################
    ...
    for epoch in range(args.epoch, epochs):
        ...
        for i, batch in tqdm(enumerate(train_loader), desc=f"Epoch: {epoch + 1}/{epochs}. Loop: Train",
            ...
            l_dense = adaptive_image_loss_func.lossfun(new_pred - depth, depth).mean() #criterion_ueff(pred, depth, mask=mask.to(torch.bool), interpolate=True)
